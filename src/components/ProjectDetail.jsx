...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const selectedProject = storedProjects.find((item) => String(item.id) === id);
    ...
    if (selectedProject) {
      setProject({
        ...
        Github: selectedProject.Github || "https://github.com/Ankit00011",
      });
    }
    ...
    return (
      <div className="min-h-screen bg-[#030014] flex items-center justify-center px-4">
        <div className="text-center space-y-6 max-w-xl">
          <h2 className="text-2xl md:text-4xl font-bold text-white">Project not found</h2>
          <p className="text-gray-400">
            This project is unavailable right now. Go back to the portfolio and
            choose another one.
          </p>
          <button
            onClick={() => navigate("/")}
            className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white/10 text-white hover:bg-white/15 transition-colors"
          >
